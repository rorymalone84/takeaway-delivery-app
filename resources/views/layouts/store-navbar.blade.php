<nav x-data="accordion(6)" x-cloak
    ...
    <div @click="handleClick()" x-data="{ open: false }" class="block text-gray-600 cursor-pointer lg:hidden flex flex-row">
        <a class=" hover:text-gray-200 pr-6" href="{{ route('cart') }}">
            ...
            <span class="flex absolute -mt-5 ml-4 ">
                @if (session('cartProducts'))
                    ...
                    <span id="totalQuantitySM"
                        class="totalQuantity relative inline-flex rounded-full h-5 w-5 bg-pink-500 text-center ">{{ session()->get('quantityTotal') }}
                    </span>
                @else
                    <span id="totalQuantitySM" class="totalQuantity">
        ...
        <div class="items-center flex-1 pt-6 justify-center text-lg text-gray-500 lg:pt-0 list-reset lg:flex">
            <a class="flex items-center hover:text-gray-200 pr-6 text-grey-700" href="{{ route('cart') }}">
                ...
                <span class="flex absolute -mt-5 ml-2">
                    @if (session('cartProducts'))
                        <span id="totalQuantityMD" class="relative inline-flex rounded-full h-9 w-9 bg-pink-500">
                            {{ session()->get('quantityTotal') }}
                        </span>
                    @else
